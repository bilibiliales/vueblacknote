...
<script>
import { signUpWithUsername } from '../utils/supabase'
...
export default {
  ...
  methods: {
    async handleRegister() {
      ...
      const { user, error } = await signUpWithUsername(this.username, this.password)
      if (error) {
        this.error = error.message || '注册失败'
        return
      }
      this.$emit('register-success', { user, username: this.username, password: this.password })
    }
  }
}
</script>
